    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < countries.Count; i++)
            {
                ...
                RunCommand(
                    $"run -ti --cap-add=NET_ADMIN --name vpn{country} -e CONNECT={country} -e USER=tkandiliotis@gmail.com  -e PASS=NordVpn123 -e TECHNOLOGY=NordLynx -d ghcr.io/bubuntux/nordvpn");

                System.Threading.Thread.Sleep(10 * 1000);

                RunCommand(
                    $"run -it --net=container:vpn{country} --name browser{country} -d curlimages/curl -L -v https://www.bitchute.com/video/Jlkoz9VOHszb/");
